...
tags:
    - Git
preview: 'Если вы только начинаете свой путь в мире разработки, Git может показаться непонятным. В этой статье я расскажу вам, как использовать Git для управления версиями ваших проектов. Мы разберем основы команд, объясним, как создавать ветки, фиксировать изменения и отправлять запросы на объединение.'
---
...
1. Создайте новый репозиторий на [GitHub](www.github.com), и выберем нашу ветку, например *master*
2. Создайте папку для вашего проекта например *project*
3. Инициализируйте гит в вашей папке *project*
```
git init
```
...
### Создадим тестовый файл README.md в директории нашего проекта.
...
### Ветки проекта, или просто branch
...
### Соеденение локального git-проекта, с GitHub
...
### Отправка нашего коммита, и просто push
...
Для смены ветки, нам нужно скачать наш проект:
```
git clone <сслыка на ваш проект>
```

И после успешной загрузки, меняем ветку:
```
...
Вы скачиваете проект:
```
git clone <ссылка на репозиторий>
```

Создаете новую ветку, чтобы не делать изменения в мастере, например вы заметили опечатку в коде:
```
git branch -M fix-typo
```

Вы вносите все нужные изменения в проект, добавляете в гит все нужные файлы (git add), и вам нужно создать коммит:
```
git commit "Исправление опечатки"
```

После создания коммита, вам нужно запушить его:
```
...
4. После разрешения конфликтов продолжите процесс перебазирования:
    ```
    git rebase --continue
    ```

5. После завершения перебазирования, ваши коммиты из ветки feature-branch будут применены поверх последнего коммита в ветке master.
...
*Данная статья может быть дополнена в будующем. Вы можете отправлять пулл реквесты правки к этой статье, ссылка на GitHub находится в шапке статьи. Спасибо!*
